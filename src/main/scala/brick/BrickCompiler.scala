    ...
    command match {
      case "init" =>
        pathOpt match {
          ...
          case None =>
            printError("Usage: brick init <path>")
        }
      case "gen" =>
        pathOpt match {
          case Some(pathStr) =>
            ...
            context.logDebug(
              s"Initializing Brick Compiler for task: $taskToRun at path: $pathStr"
            )
            try {
              ...
              for (bricks <- tree) {
                ...
                context.logInfo(
                  s"Compiling ${bricks.name} with ${bricks.bricks.size} bricks"
                )
          ...
          case None =>
            printError("Usage: brick gen <path>")
        }
      case _ =>
        printError(
          "Unknown command. Use 'brick init <path>' or 'brick gen <path>'"
        )
    }
  }
}
